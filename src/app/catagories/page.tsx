...
//           gender: formData.gender,
//           imageBase64: imageBase64,
//           serviceCount: editingCategory.serviceCount
//         });
//       } else {
//         await addCategory({
//           name: formData.name,
...
//     <div className="bg-white/90 backdrop-blur-xl border border-pink-200/30 rounded-xl sm:rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(233,30,99,0.15)] transition-all duration-300 hover:shadow-[0_12px_40px_rgb(233,30,99,0.25)] hover:scale-[1.02] group">
//       {/* Image & Content */}
//       {/* ... (existing image & content rendering code stays unchanged) */}
//     </div>
//   );
// }
...
//   const resetForm = () => {
//     setFormData({ name: '', description: '', color: 'from-pink-400 to-pink-500', image: '', gender: 'men' });
//     setImageFile(null);
//     setShowModal(false);
//     setEditingCategory(null);
//   };
...
//       catch { alert('Error deleting category. Please try again.'); }
//     }
//   };
...
function CategoryCard({ category, onEdit, onDelete }: {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-xl border border-pink-200/30 rounded-xl sm:rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(233,30,99,0.15)] transition-all duration-300 hover:shadow-[0_12px_40px_rgb(233,30,99,0.25)] hover:scale-[1.02] group">
      <div className="relative h-20 sm:h-24 overflow-hidden">
        {category.imageBase64 ? (
          <Image src={category.imageBase64} alt={category.name} fill className="object-cover"/>
        ) : (
          <div className={`h-full bg-gradient-to-br ${category.color} relative`}>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-white/80 text-xs sm:text-sm">📂</span>
        ...
        <div className="absolute top-1 right-1 flex space-x-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
          <button onClick={() => onEdit(category)}
            className="w-5 h-5 sm:w-6 sm:h-6 bg-white/95 backdrop-blur-sm rounded-md flex items-center justify-center text-pink-600 hover:bg-white hover:scale-110 text-xs transition-all shadow-md border border-white/20">✎</button>
          <button onClick={() => onDelete(category)}
            className="w-5 h-5 sm:w-6 sm:h-6 bg-white/95 backdrop-blur-sm rounded-md flex items-center justify-center text-pink-600 hover:bg-red-50 hover:text-red-600 hover:scale-110 text-xs transition-all shadow-md border border-white/20">×</button>
        </div>
      </div>
      <div className="p-2 sm:p-3">
        <h3 className="text-xs font-semibold text-pink-700 leading-tight truncate mb-1">{category.name}</h3>
        <p className="text-xs text-pink-500 leading-tight line-clamp-2">{category.description}</p>
        <span className={`inline-block mt-2 px-2 py-0.5 rounded-full text-xs font-medium ${getGenderBadgeColor(category.gender)}`}>{category.gender}</span>
      </div>
    </div>
  );
}
